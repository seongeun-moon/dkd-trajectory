...
def obtain_attention_score(q, k):
    k_cumsum = k.sum(dim=-2)
    D_inv = 1. / torch.einsum('...nd,...d->...n', q, k_cumsum.type_as(q))
    batch_size, nheads, nfeatures, _ = k.size()
    v = torch.eye(nfeatures).reshape(1, 1, nfeatures, nfeatures).to(q.device)
    v = torch.repeat_interleave( torch.repeat_interleave(v, nheads, 1), batch_size, 0)
    context = torch.einsum('...nd,...ne->...de', k, v)
    out = torch.einsum('...de,...nd,...n->...ne', context, q, D_inv)
    return out[:, :, 0, :]
...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v, return_attn_score=False):
        device = q.device
        ...
        attn_score = obtain_attention_score(q, k) if return_attn_score else torch.zeros(1).to(device)
...
class ProjectionUpdater(nn.Module):
    def __init__(self, instance, feature_redraw_interval, device):
        super().__init__()
    ...
    def redraw_projections(self):
        ...
        if exists(self.feature_redraw_interval) and self.calls_since_last_redraw >= self.feature_redraw_interval:
            ...
            for fast_attention in fast_attentions:
                fast_attention.redraw_projection_matrix(self.device)
...
class PreLayerNorm(nn.Module):
    def __init__(self, dim, fn):
        ...
        self.fn = fn
    def forward(self, x, attn_score=None, **kwargs):
        if attn_score is None:
            y = self.fn(self.norm(x), **kwargs)
        else:
            y = self.fn(self.norm(x), attn_score, **kwargs)
        return y
...
class Attention(nn.Module):
    ...
    def forward(self, x, pos_emb = None, context = None, mask = None, context_mask = None, **kwargs):
        ...
        if not empty(q):
            ...
            out, score = self.fast_attention(q, k, v, **kwargs)
...
class Performer(nn.Module):
    def __init__(
        ...
        self.proj_updater = ProjectionUpdater(self.net, feature_redraw_interval, device)
